...
export class UsersService {
  ...
  async createUser(email: string, userId: string): Promise<User> {
    const newUser = await this.usersRepository.createUser(email, userId);
  ...
  async getUserAvatar(userId: string): Promise<string> {
    try {
      ...
      if (user && user.avatar) {
        return user.avatar; // Return the base64-encoded representation from the database
      }
      ...
      if (!userData || !userData.avatar) {
        throw new InternalServerErrorException(
          'User data from external API is invalid',
        );
      }
