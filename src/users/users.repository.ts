...
export class UsersRepository {
  ...
  async createUser(email: string, userId: string): Promise<User> {
    const newUser = new this.userModel({ email, userId });
    return await newUser.save();
  }

  // Define an asynchronous method to find a user by their ID
  ...
  async updateUserAvatar(
    ...
    try {
      console.log('Updating user avatar for userId:', userId); // Log userId for debugging
      ...
      console.error('Error updating user avatar:', error); // Log error for debugging
